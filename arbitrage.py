...
CONFIG = {
    ...
    "trade_amount": 5,        # Amount in base_currency to trade (changed to $5)
    "max_trade_count": 10,    # Maximum number of trades to execute
    "test_mode": True,        # Set to False to execute real trades
    "update_interval": 6.0,   # Time in seconds between market checks (increased to 6 seconds)
    "fee_rate": 0.1,          # Trading fee percentage (0.1 = 0.1%)
    "max_retries": 3,         # Maximum number of retries for API calls
    "enable_rollback": True,  # Enable automatic rollback of incomplete trades
    "backoff_factor": 2,      # Factor for exponential backoff (wait time multiplier)
    "jitter": 0.1,            # Random jitter factor to add to backoff times (0.1 = 10%)
}
...
class TriangularArbitrageBot:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Rollback: {'Enabled' if self.enable_rollback else 'Disabled'}")
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.info(f"Loaded information for {len(self.symbols_info)} trading pairs")
    ...
    def find_potential_triangles(self):
        ...
        logger.info(f"Found {len(self.potential_triangles)} potential triangular paths")
        ...
        while retries <= self.max_retries:
            try:
                ...
                if e.code == 429 or e.code == 1013:
                    ...
                    logger.warning(f"Rate limit hit (code {e.code}). Retrying in {wait_time:.2f} seconds. Retry {retries+1}/{self.max_retries}")
                ...
                last_exception = e
                wait_time = self.calculate_backoff_time(retries)
                logger.warning(f"API error: {e}. Retrying in {wait_time:.2f} seconds. Retry {retries+1}/{self.max_retries}")
                time.sleep(wait_time)
                retries += 1
    ...
    def update_prices(self):
        ...
        try:
            ...
            self.ticker_prices = {item['symbol']: float(item['price']) for item in prices}
            return True
        except Exception as e:
    ...
    def calculate_profit(self, triangle):
        ...
        try:
            ...
            profit_percent = ((step3_amount - self.trade_amount) / self.trade_amount) * 100
    ...
    def find_arbitrage_opportunities(self):
        ...
        if not self.update_prices():
            logger.warning("Failed to update prices, skipping opportunity check")
            return []
    ...
    def execute_arbitrage(self, opportunity):
        ...
        if self.test_mode:
            logger.info(f"TEST MODE: Would execute trade with {opportunity['profit_percent']:.2f}% profit")
            logger.info(f"Step 1: {opportunity['triangle']['step1']['operation']} {opportunity['triangle']['step1']['symbol']}")
            logger.info(f"Step 2: {opportunity['triangle']['step2']['operation']} {opportunity['triangle']['step2']['symbol']}")
            logger.info(f"Step 3: {opportunity['triangle']['step3']['operation']} {opportunity['triangle']['step3']['symbol']}")
        ...
        try:
            ...
            quantity1 = self.calculate_quantity(
                step1['symbol'], 
                opportunity['amounts']['start'] / opportunity['prices']['price1']
            )
            ...
            logger.info(f"Step 1 complete: {step1['operation']} {quantity1} {step1['symbol']}")
            
            # Update active trade status
            self.active_trade['orders']['step1'] = order1
            self.active_trade['status'] = 'completed_step1'
            self.active_trade['assets_held'][step1['to']] = float(order1['executedQty'])
            ...
            logger.info(f"Step 2 complete: {step2['operation']} {quantity2} {step2['symbol']}")
            
            # Update active trade status
            self.active_trade['orders']['step2'] = order2
            self.active_trade['status'] = 'completed_step2'
            self.active_trade['assets_held'][step2['to']] = float(order2['cummulativeQuoteQty'])
            ...
            logger.info(f"Step 3 complete: {step3['operation']} {quantity3} {step3['symbol']}")
            
            # Update active trade status
            self.active_trade['orders']['step3'] = order3
            self.active_trade['status'] = 'completed_step3'
            self.active_trade['assets_held'][step3['to']] = float(order3['cummulativeQuoteQty'])
            
            # Record successful trade
            self.trade_count += 1
            final_amount = float(order3['cummulativeQuoteQty'])
            actual_profit = final_amount - opportunity['amounts']['start']
            actual_profit_percent = (actual_profit / opportunity['amounts']['start']) * 100
            ...
            logger.info(f"Arbitrage complete! Profit: {actual_profit:.2f} {self.base_currency} ({actual_profit_percent:.2f}%)")
            ...
            if self.enable_rollback and self.active_trade:
                logger.info(f"Attempting to rollback incomplete trade at step {self.active_trade['current_step']}")
    ...
    def rollback_incomplete_trade(self):
        ...
        try:
            ...
            logger.info(f"Rolling back from step {current_step} with assets: {assets_held}")
            ...
            if current_step == 1 and opportunity['triangle']['step1']['to'] in assets_held:
                ...
                if symbol in self.symbols_info:
                    logger.info(f"Rolling back: Selling {token_amount} {token} for {self.base_currency}")
                    ...
                    logger.info(f"Rollback complete: Sold {token} back to {self.base_currency}")
                    return True
                else:
                    logger.warning(f"Cannot rollback: No direct pair {symbol} exists")
            
            # If we failed after step 2 (holding alt_currency)
            elif current_step == 2 and opportunity['triangle']['step2']['to'] in assets_held:
                alt_amount = assets_held[opportunity['triangle']['step2']['to']]
                symbol = opportunity['triangle']['step3']['symbol']
                
                logger.info(f"Rolling back: Selling {alt_amount} {self.alt_currency} for {self.base_currency}")
                ...
                logger.info(f"Rollback complete: Sold {self.alt_currency} back to {self.base_currency}")
                return True
            ...
            for balance in account['balances']:
                ...
                if symbol in self.symbols_info and free_amount > 0:
                    ...
                    try:
                        ...
                        if float(quantity) > 0:
                            logger.info(f"Selling stranded asset: {quantity} {asset}")
                            ...
                            logger.info(f"Successfully sold stranded {asset}: {order['executedQty']} for {order['cummulativeQuoteQty']} {self.base_currency}")
                    except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            while self.trade_count < self.max_trade_count:
                logger.info(f"Looking for arbitrage opportunities... ({self.trade_count}/{self.max_trade_count})")
                ...
                if opportunities:
                    ...
                    logger.info(f"Found opportunity with {best_opportunity['profit_percent']:.2f}% profit")
                    
                    # Execute the trade
                    success = self.execute_arbitrage(best_opportunity)
                    
                    if success:
                        logger.info(f"Successfully executed trade {self.trade_count}/{self.max_trade_count}")
                    else:
                        logger.warning("Failed to execute trade, will try again")
            ...
            logger.info(f"Completed {self.trade_count} trades. Shutting down...")
            self.save_results()
...
if __name__ == "__main__":
    ...
    if CONFIG["test_mode"]:
        ...
        confirm = input("You are about to run this bot in LIVE MODE. Are you sure? (yes/no): ")
